...
import { assertLockfileConsistency } from './checks/lockfileConsistency.mjs'
...
const RequiredPublishedFiles = [
  ...
  'dist/node.js',
  'dist/node.d.ts',
  'dist/provider-scripts/node.js',
  'dist/provider-scripts/node.d.ts',
]
  ...
  const nestedFiles = await Promise.all(
    entries.map((entry) => {
      ...
      return entry.isDirectory()
        ...
        : Promise.resolve(predicate(entryPath) ? [entryPath] : [])
    })
  )
...
async function auditPackageRoot(
  ...
  if (repositoryOwned) {
    ...
    const contractsRuntime = await readFile(
      join(packageRoot, 'dist/contracts.js'),
      'utf8'
    )
    assert.doesNotMatch(contractsRuntime, /\b(?:import|from)\b/u)
    assert.doesNotMatch(
      contractsRuntime,
      /\bexport\s+(?:class|const|function|let|var)\b/u
    )
    assert.doesNotMatch(contractsRuntime, /export\s*\{[^}]*\S[^}]*\}/u)
...
async function writeConsumerProject(consumerDirectory) {
  await writeFile(
    ...
    JSON.stringify(
      {
        name: 'velaros-model-package-consumer',
        private: true,
        type: 'module',
      },
      null,
...
process.env.VELAROS_OLLAMA_CHAT_MODEL = 'ambient-model'
assert.equal(catalog.resolveOllamaChatModel('fallback-model'), 'fallback-model')
assert.equal(
  new catalog.LocalModelEnvironment({
    VELAROS_OLLAMA_CHAT_MODEL: 'qwen3:8b',
  }).resolveOllamaChatModel('fallback-model'),
  'qwen3:8b'
)
...
assert.equal(typeof nodeRuntime.createModelAdapterRegistry, 'function')
assert.equal(typeof nodeRuntime.createLegacyModelAdapterRegistry, 'function')
assert.equal(typeof contextWindow.resolveProviderScriptContextWindow, 'function')
`
  )
}
  ...
  plugins: [
    {
      ...
      resolveId(source) {
        if (NodeBuiltins.has(source)) {
          throw new Error(
            \`Browser-safe package root imported Node builtin: \${source}\`
          )
        }
        return null
      },
...
async function assertBrowserBundleOutput(consumerDirectory) {
  const outputDirectory = join(consumerDirectory, 'browser-dist')
  const outputFiles = await collectFiles(outputDirectory, () => true)
  assert.ok(outputFiles.length > 0, 'Vite browser gate did not emit a bundle')

  for (const outputPath of outputFiles) {
    const source = await readFile(outputPath, 'utf8')
    assert.doesNotMatch(
      source,
      /(?:__vite-browser-external|from\s*['"]node:|import\s*\(\s*['"]node:)/u,
      `Browser bundle contains an externalized Node builtin: ${outputPath}`
    )
  }
}
...
async function main() {
  const temporaryRoot = await mkdtemp(join(tmpdir(), 'velaros-model-consumer-'))
  ...
  try {
    ...
    await writeBrowserBundleGate(consumerDirectory)
    run(process.execPath, [TypeScriptCli, '--project', 'tsconfig.json'], {
      cwd: consumerDirectory,
    })
    run(process.execPath, ['runtime-import.mjs'], { cwd: consumerDirectory })
    run(process.execPath, [ViteCli, 'build', '--config', 'vite.config.mjs'], {
      cwd: consumerDirectory,
    })
    await assertBrowserBundleOutput(consumerDirectory)
    ...
    console.info(
      '✓ Model package passes isolated registry-backed standalone and Core/UI combined consumer gates'
    )
    if (SkipLockfileAudit) {
