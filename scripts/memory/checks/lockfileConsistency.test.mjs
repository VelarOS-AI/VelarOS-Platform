...
test('restores exact manifest bytes and fails if Bun mutates any manifest', () => {
  const repositoryRoot = createFixture()
  ...
  assert.throws(
    () =>
      new GroupedPrivateLockRefresher({
        repositoryRoot,
        runUpdate: () => writeFileSync(manifestPath, '{ "changed": true }\n'),
      }).refresh(),
    /original bytes restored: packages\/example\/package\.json/,
  )
