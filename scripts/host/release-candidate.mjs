...
function parseArguments(argv) {
  ...
  if (!new Set(['plan', 'stage', 'finalize']).has(command)) {
    throw new Error('Expected Host release command: plan, stage, or finalize')
  }
  const options = {
    command,
    channel: 'stable',
...
async function releaseIdentity() {
  ...
  if (typeof manifest.version !== 'string')
    throw new Error('Host version is missing')
  if (
    !sourceRepository ||
    !/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/u.test(sourceRepository)
  ) {
    throw new Error(
      'Host release requires GITHUB_REPOSITORY or VELAROS_SOURCE_REPOSITORY',
    )
  }
  if (!sourceCommit || !/^[a-f0-9]{40}$/u.test(sourceCommit)) {
    throw new Error('Host release requires an exact 40-character source commit')
  }
  return {
    product: 'host',
    version: manifest.version,
    tag: `host-v${manifest.version}`,
    sourceRepository,
    sourceCommit,
  }
}
...
function buildCandidateManifest({ identity, channel, stages }) {
  ...
  for (const platform of platforms) {
    ...
    if (!stage) throw new Error(`Missing staged Host installer for ${platform}`)
    for (const field of [
      'product',
      'version',
      'tag',
      ...
      if (stage[field] !== expected) {
        throw new Error(
          `Staged Host ${platform} ${field} does not match this release`,
        )
      }
    }
  }
...
class GitHubReleaseClient {
  ...
  async request(url, options = {}) {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'User-Agent': 'velaros-host-release',
        ...options.headers,
      },
    })
  ...
  async assertPrivate() {
    ...
    if (repository.private !== true) {
      throw new Error(
        `Refusing to upload Host installers to non-private ${this.repository}`,
      )
    }
  }
    ...
    return this.request(
      `https://api.github.com/repos/${this.repository}/releases`,
      {
        ...
        body: JSON.stringify({
          ...
          name: `Velar Host v${identity.version} candidate`,
          body: `Private Velar Host candidate from ${identity.sourceRepository}@${identity.sourceCommit}.`,
          draft: false,
          prerelease: true,
          make_latest: 'false',
        }),
  ...
  async uploadBytes(
    ...
    if (current) {
      ...
      if (!replaceExisting) {
        throw new Error(
          `${identityLabel(release)} already has different bytes for ${fileName}; bump the Host version`,
        )
      }
...
async function plan(options) {
  ...
  const markerName = `candidate-host-${options.channel}.json`
  ...
  if (candidate.sourceCommit !== identity.sourceCommit) {
    throw new Error(
      `${identity.tag} is already complete from ${candidate.sourceCommit}; bump the Host version`,
    )
  }
  await output('host_needed', false)
...
async function stage(options) {
  ...
  for (const [field, expected] of Object.entries({
    ...
    if (artifactManifest[field] !== expected) {
      throw new Error(
        `Host artifact manifest ${field} does not match: expected ${expected}`,
      )
    }
  }
  ...
  if (
    ...
    throw new Error('Host artifact bytes do not match the build manifest')
  }
  ...
  const stageName = `staged-host-${options.platform}.json`
  await client.uploadBytes(
    release,
    stageName,
...
async function finalize(options) {
  ...
  for (const platform of platforms) {
    const staged = await client.readAsset(
      release,
      `staged-host-${platform}.json`,
    )
    if (!staged)
      throw new Error(`Missing staged Host installer for ${platform}`)
  ...
  const outputRoot = resolve(repoRoot, 'dist-host', 'release')
  const fileName = `candidate-host-${options.channel}.json`
