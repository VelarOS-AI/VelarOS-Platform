...
function findSymbolRange(input: PatchStrategyInput): { start: number; end: number } {
  ...
  if (!symbolRef || !input.snapshot) throw new WorkspaceError("TARGET_NOT_FOUND", "replace_symbol 需要 targetId，或 operation.symbol 加 snapshot");
  ...
  if (matches.length !== 1) {
    throw new WorkspaceError(isEmpty(matches) ? "TARGET_NOT_FOUND" : "AMBIGUOUS_TARGET", `replace_symbol 预期 1 个匹配，实际找到 ${matches.length} 个`);
  }
...
function replaceSymbol(input: PatchStrategyInput): PreparedPatch[] {
  if (!input.snapshot || input.snapshot.isBinary) throw new WorkspaceError("NOT_SUPPORTED", "replace_symbol 需要 JS/TS 文本 snapshot");
  const op: any = input.intent.operation;
  const content = input.snapshot.content ?? "";
  ...
  if (op.mode === "body" || op.replaceBodyOnly) {
    ...
    if (!bodyRange) throw new WorkspaceError("TARGET_NOT_FOUND", "replace_symbol mode=body 需要可解析的函数或方法 body 范围");
...
function insertAroundSymbol(input: PatchStrategyInput): PreparedPatch[] {
  if (!input.snapshot || input.snapshot.isBinary) throw new WorkspaceError("NOT_SUPPORTED", "insert_around_symbol 需要 JS/TS 文本 snapshot");
  const op: any = input.intent.operation;
  const content = input.snapshot.content ?? "";
...
function addImport(input: PatchStrategyInput): PreparedPatch[] {
  if (!input.snapshot || input.snapshot.isBinary) throw new WorkspaceError("NOT_SUPPORTED", "add_import 需要 JS/TS 文本 snapshot");
  const op: any = input.intent.operation;
  const content = input.snapshot.content ?? "";
  const parsed = parseTs(input.snapshot.path, content);
...
function removeImport(input: PatchStrategyInput): PreparedPatch[] {
  if (!input.snapshot || input.snapshot.isBinary) throw new WorkspaceError("NOT_SUPPORTED", "remove_import 需要 JS/TS 文本 snapshot");
  const op: any = input.intent.operation;
  const content = input.snapshot.content ?? "";
  const parsed = parseTs(input.snapshot.path, content);
  const imports = parsed.symbols.filter((s) => s.kind === "import" && (!op.module || s.name === op.module));
  if (isEmpty(imports)) throw new WorkspaceError("TARGET_NOT_FOUND", `未找到 import：${op.module ?? op.name ?? "<any>"}`);
  if (imports.length > 1 && !op.module) throw new WorkspaceError("AMBIGUOUS_TARGET", `remove_import 找到 ${imports.length} 个 import；请指定 module`);
  ...
  if (op.name) {
    ...
    if (!braceMatch) throw new WorkspaceError("TARGET_NOT_FOUND", `未找到 named import：${op.name}`);
    const current = braceMatch[1].split(",").map((s: string) => s.trim()).filter(Boolean);
    const next = current.filter((name: string) => name !== op.name);
    if (next.length === current.length) throw new WorkspaceError("TARGET_NOT_FOUND", `未找到 named import：${op.name}`);
    if (!isEmpty(next)) {
...
export function typescriptPatchStrategy(): PatchStrategy {
  return {
    ...
    prepare(input: PatchStrategyInput): PreparedPatch[] {
      ...
      throw new WorkspaceError("NOT_SUPPORTED", `不支持的 TypeScript patch 操作：${type}`);
    },
  };
}
