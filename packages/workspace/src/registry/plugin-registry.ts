...
import type { WorkspaceFixer } from "../types/fix.js";
...
import type { WorkspacePlugin, WorkspacePluginContext } from "../types/plugin.js";
import type { WorkspaceValidator } from "../types/validation.js";

/** plugin 安装时可写入的注册入口，由 Workspace 实例实现。 */
...
export class PluginRegistry {
  private plugins: WorkspacePlugin[] = [];
  private pluginNames = new Set<string>();

  /** 安装一个插件，并把插件注册的能力写入目标 registry。 */
  public async install(plugin: WorkspacePlugin, sink: RegistrySink): Promise<void> {
    if (this.pluginNames.has(plugin.name)) return;
    this.pluginNames.add(plugin.name);
    // 只暴露绑定过的注册函数，避免 plugin 直接依赖 Workspace 具体实现。
    const ctx: WorkspacePluginContext = {
