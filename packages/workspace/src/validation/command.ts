...
import type { FixInput, FixResult, WorkspaceFixContext, WorkspaceFixer } from "../types/fix.js";
...
export interface CommandValidationContext {
  root: string;
  providers: WorkspaceProviders & { command: CommandProvider };
}
...
export function commandValidator(options: CommandValidatorOptions): WorkspaceValidator {
  return {
    id: options.id,
    canValidate(input) {
...
export function eslintFixer(args = ["eslint"]): WorkspaceFixer {
