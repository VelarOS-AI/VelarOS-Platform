...
export interface WorkspaceIntelligenceSymbol {
  name: string;
  kind: string;
  path: string;
  line: number;
...
export interface WorkspaceIntelligenceLocation {
  path: string;
  line: number;
  column: number;
  endLine: number;
...
/** Narrow injectable code-intelligence surface consumed by Workspace. */
export interface WorkspaceCodeIntelligenceHost {
  readonly available: boolean;
  isProjectEnabled(projectRoot: string): boolean;
  listSymbols(input: {
    projectRoot: string;
    path: string;
  }): Promise<WorkspaceIntelligenceSymbol[]>;
  getDefinition(input: {
    projectRoot: string;
    path: string;
    line: number;
    column: number;
    symbol?: string;
  }): Promise<WorkspaceIntelligenceLocation[]>;
}
...
export interface ContextProvider {
  ...
  /** evidence 离开 workspace 边界前的最后清洗步骤。 */
...
export interface WorkspaceProviders {
  ...
  codeIntelligence?: WorkspaceCodeIntelligenceHost;
}
