...
function decode<TFrame>(
  ...
  if (!(raw instanceof Buffer) && !isString(raw)) {
    return { ok: false, reason: 'Frame payload is neither text nor binary' }
  }
  const text = raw.toString()
  if (Buffer.byteLength(text, 'utf8') > RemoteNodeMaxFrameBytes) {
    return { ok: false, reason: 'Frame exceeds the protocol size limit' }
  }
  let input: unknown
  try {
  ...
  if (!parsed.success || isUndefined(parsed.data)) {
    return { ok: false, reason: 'Frame does not match the protocol schema' }
  }
