...
const convertPdfToWord = defineOfficeTool<ConvertPdfToWordInput>({
  ...
  execute: async (input: ConvertPdfToWordInput, ctx: OfficeToolContext) => {
    ctx.abortSignal.throwIfAborted()
    // 生成 docx 属于写操作，先让 workspace 做权限/路径确认。
    const authorization = await ctx.workspace.prepareMutationWorkspace({
      cwd: input.cwd,
...
const pdfTools = {
  convert_pdf_to_word: convertPdfToWord,
  convert_word_to_pdf: convertWordToPdf,
  create_latex_pdf: createLatexPdf,
  edit_pdf_document: editPdfDocument,
}
