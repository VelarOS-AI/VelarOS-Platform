...
import { knowledgeTools } from './index'
import type { KnowledgeToolContext } from './Types'

export type { VelarosCliRunOptions as RunKnowledgeCliOptions }

export async function runKnowledgeCli(
  argv: string[],
  options: VelarosCliRunOptions = {}
): Promise<VelarosCliRunResult> {
  return runToolCollectionCli<KnowledgeToolContext>(argv, {
    namespace: 'knowledge',
    binName: 'velaros-knowledge',
    tools: knowledgeTools as any,
    cwd: options.cwd,
    createContext: () => ({
      abortSignal: new AbortController().signal,
      sessionId: 'local-cli',
      knowledge: createHostRequiredProxy('knowledge') as any,
...
export async function main(
  ...
  const result = await runKnowledgeCli(argv, { cwd })
