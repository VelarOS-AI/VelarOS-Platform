...
import { createCallbackTransport } from '@velaros-ai/core/logger'
import { createDefaultKernelDaemonPaths } from '@velaros-ai/kernel/client/contracts'

import { bootKernelDaemon } from '../../../src/serve/daemon/boot'
import { KernelModLoader } from '../../../src/serve/daemon/mod-loader'
import { type KernelModPackRecord, KernelModStore } from '../../../src/serve/daemon/mod-store'

const directories: string[] = []
let logCaptureSequence = 0
...
function captureErrorLog(): { lines: string[]; restore: () => void } {
  const lines: string[] = []
  const id = `kernel-mod-loader-test-${++logCaptureSequence}`
  Log.addTransport(createCallbackTransport({
    id,
    accepts: (record) => record.kind === 'message'
      && record.scope === 'KernelModLoader'
      && record.level === 'error',
    write: (record) => {
      lines.push(JSON.stringify(record))
    },
  }))
  return { lines, restore: () => { Log.removeTransport(id) } }
}
