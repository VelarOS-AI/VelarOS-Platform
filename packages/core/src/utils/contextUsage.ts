import { isArray, isBoolean, isFiniteNumber,isNumber, isObject, isPresent, isString } from '../typeGuards.js'
...
export interface ContextUsageEstimate {
  ...
  /**
   * 模型真实上下文窗口。现与 contextWindow 恒等(2026-07-18 起水位按真实窗口计);
   * 保留双字段是因为旧落盘估算里 contextWindow 是 200K 治理钳制口径、二者不等,
   * 展示层靠对比二字段决定是否渲染"压缩水位/治理窗口"附加行。
   */
    ...
    Object.entries(record).map(([key, entry]) => {
      if (key === 'imageAttachments' && isArray(entry)) return [
          ...
          entry.map((attachment) => {
            if (!attachment || !isObject(attachment)) return attachment
...
function estimateValueTokens(value: unknown, seen: Set<object>): number {
  ...
  if (!isObject(value)) return estimateStringTokens(String(value))
...
export function estimateContextUsage(
  ...
  const modelContextWindow = isFiniteNumber(options.contextWindow)
      ? Math.max(1, Math.floor(options.contextWindow))
      : 128_000
  // 水位一律按模型真实窗口百分比计(2026-07-18 用户裁决,推翻旧 min(模型窗口,200K) 治理
  // 钳制):每请求体积另有注意力路由器/microCompaction/回收阶梯独立约束,硬钳制会让 1M
  // 容量对会话不可用(长内容被提前折叠/裁剪)。contextWindow 与 modelContextWindow 因此
  // 恒等;两字段都保留——旧落盘估算里二者不等,展示层的治理拆分要靠对比才能正确渲染。
