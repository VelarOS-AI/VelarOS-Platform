import { isFalse } from '../typeGuards.js'
import type {
  TurnContextDelta,
...
export class TurnContextLedger {
  ...
  public peek(input: Pick<TurnContextSourcePeekInput, 'afterSeq' | 'generation'>): TurnContextLedgerPeekResult {
    ...
    const headSeq = this.items.length > 0 ? this.items[0]!.seq : this.nextSeq

    // 代次不匹配（重启后持久化旧 cursor）：不回放旧 delta，只交出当前基底。
    if (input.generation !== null && input.generation !== this.generation) return {
    ...
    const hasGap = this.droppedBeforeSeq !== null && afterSeq + 1 < this.droppedBeforeSeq
