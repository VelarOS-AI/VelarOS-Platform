...
`@velaros-ai/agent` owns host-agnostic agent execution: loop runners, prompt state, role routing, orchestration policy, execution ledgers, team planning, capability consumer ports, tool registry/executor helpers, reminders, and runtime event contracts.
...
```ts
import type { AgentRuntimeCapabilityPorts } from '@velaros-ai/agent'
