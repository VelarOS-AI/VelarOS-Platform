...
当前稳定入口为 `@velaros-ai/agent`。根入口按领域导出 `agent`、`capabilities`、`execution`、`kernel`、`prompts`、`skills`、`team`、`tools` 和 `workflow` API。不要导入源码或 `dist` 私有路径。
...
import { MonotonicExecutionIdFactory } from '@velaros-ai/agent'
