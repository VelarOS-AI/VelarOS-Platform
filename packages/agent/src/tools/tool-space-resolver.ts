// 域：工具**能力作用域可见性**解析（哪些能力描述符在当前 scope/policy/runtime profile 下进入调用目录）。
//
...
//  - **类别闸门**：并列作用域按类别隔离，一个作用域看不到另一个作用域的工具族。
//
// ## 关键不变量（改这些会破什么）
//  - **身份走声明、可用性走运行态**：作用域身份由 descriptor 声明决定，宿主资源是否已绑定只影响
//    可用性。历史事故：两者合取 → 资源未绑定时该作用域的工具全隐身，而绑定动作本身又需要那些
//    工具 = 死锁。
