...
export class ToolExecutor {
  ...
  private onProviderTurnSnapshot: LooseOptional<(snapshot: ProviderTurnSnapshot) => void>
  ...
  constructor(
    ...
    this.materializer = new KernelToolResultMaterializer({
      outputStore: toNullable(options.outputStore) ?? this.createContextPayloadOutputStore(),
    })
  ...
  public enqueue(
    ...
    const canonicalConcurrencySafe =
      !providerArgs.ok
        ? false
        : canonicalToolName === toolName
        ? isConcurrencySafe
        : this.executionPolicy.resolveConcurrencySafe(canonicalToolName, pendingArgs)
  ...
  public async collectAll(): Promise<ToolResult[]> {
    ...
    const results = this.tools.filter((t) => isPresent(t.result)).map((t) => t.result!)
  ...
  private async runOne(tool: PendingTool): Promise<ToolResult> {
    ...
    if (this.seams?.has('tool-call:before')) {
      ...
      if (outcome.block) {
        ...
        return this.finalizeResult(tool, {
          ...
          toolName: tool.toolName,
          error: reason,
          result: this.buildToolFailureResult('tool_blocked', reason, tool.toolName),
        })
      }
    }

    const startedAt = Date.now()
    log.info('tool execute start', { name: tool.toolName, id: tool.toolCallId })
    ...
    this.siblingAbort.signal.addEventListener('abort', siblingOff, { once: true })
    ...
    try {
      ...
      const appError = this.buildExecutionAppError(err, activeExecutionContext?.toolContext)
      ...
      terminalError = this.applyExecutionFailureSideEffects({
        ...
        isConcurrencySafe: activeExecutionContext?.isConcurrencySafe ?? tool.isConcurrencySafe,
      ...
      result = {
        toolCallId: tool.toolCallId,
        toolName: tool.toolName,
        result: this.executionPolicy.buildExecutionFailureResult(
  ...
  private applyExecutionFailureSideEffects(input: {
    ...
    const terminalError = this.executionPolicy.isTerminalExecutionError(input.error)
      ? input.error
      : null
  ...
  private async executePreparedToolWithAbortSettlement(
    ...
    const execution = this.executePreparedTool(tool, prepared, args, toolName).then(
    ...
    try {
      const settlement = await Promise.race([execution, abortSettlement.promise])
      switch (settlement.status) {
        ...
        case 'aborted': {
          ...
          return {
            ...
            toolName,
            error: reason,
            result: this.buildToolFailureResult('tool_cancelled', reason, toolName, {
              details: {
                graceMs: ToolAbortSettlementGraceMs,
              },
            }),
  ...
  private createAbortSettlement(abortSignal: AbortSignal): {
    ...
    const promise = new Promise<ToolExecutionSettlement>((resolve) => {
      const scheduleSettlement = (): void => {
        ...
        timers.after(ToolAbortSettlementGraceMs, () => resolve({ status: 'aborted' }), {
  ...
  private resolveAbortSettlementReason(): string {
    if (this.siblingErrored && this.siblingAbort.signal.aborted) return `Cancelled: sibling tool "${this.siblingErrorName}" failed`

    return ToolCancelledByUserReason
  }

  private async finalizeResult(tool: PendingTool, result: ToolResult): Promise<ToolResult> {
  ...
  private createContextPayloadOutputStore(): Nullable<KernelToolOutputStore> {
    const payloadStore = (this.ctx as ToolExecutorPayloadContext).contextPayloadStore
  ...
  private emitProviderTurnSnapshot(): void {
    ...
    try {
      const completion = completeProviderTurnSnapshot(this.providerTurnReducer, {
  ...
  private readProviderToolArgs(
    ...
    if (isPlainObject(args)) return { ok: true, args: args as Record<string, unknown> }
    ...
    return {
      ok: false,
      reason,
      result: this.buildToolFailureResult('schema_validation_failed', reason, toolName, {
        code: 'VALIDATION',
        details: {
          receivedType,
  ...
  private applyRepeatedFailureBatchGuard(results: ToolResult[]): void {
    ...
    const reason = [target.error, `[loop guard] ${message}`].filter(isString).join('\n\n')
    const guardedResult = this.buildToolFailureResult('tool_loop_guard', reason, target.toolName)
