...
import { isEmpty, isNotNull, isNull, isObject, isPresent, isString, toNullable } from '@velaros-ai/core'
...
import { ToolExecutionPolicy, type ToolExecutionPolicyRegistry } from '../../tools'
...
import { readGeneratedFilePayload, readSourcePayload } from './generated-artifacts'
import { readNextStreamPartWithIdleTimeout, returnStreamIteratorQuietly } from './idle-read'
import {
...
class StreamConsumer {
  ...
  public async consumeAssistantStream(
    ...
    const idleReconnectNotifier = this.createIdleReconnectNotifier(args, turnState)
    let currentText = ''
    ...
    const emitInlineReasoningDelta = (text: string): void => {
      ...
      // 「补一个可见答复」的恢复逻辑。
      args.providerTurnReducer?.apply({ type: 'assistant-reasoning-delta', text })
      args.events.emitReasoningDelta({ id: InlineReasoningTagStreamId, text })
    }
    const routeInlineReasoningSegments = (segments: InlineReasoningSegment[]): void => {
    ...
    const emitAssistantTextDelta = (rawText: string): void => {
      const visible = filterLeakedToolMarkupDelta(leakedToolMarkupState, rawText)
      if (!visible) return
      routeInlineReasoningSegments(splitInlineReasoningTagDelta(inlineReasoningTagState, visible))
    }
    ...
    if (args.providerTurnReducer && args.contextEpoch) {
      recordKernelContextEpochDiagnostic(args.providerTurnReducer, args.contextEpoch, {
    ...
    const flushPendingToolCalls = (): void => {
      ...
      for (const toolCall of pendingToolCalls) {
        ...
        args.events.emitToolStart({
          ...
          toolName: toolCall.toolName,
          args: toolCall.input,
          categoryId: this.executionPolicy.getToolCategoryId(toolCall.toolName) || undefined,
        })
        if (toolCall.streamedInput) {
          args.events.emitToolMetadata?.({
            toolCallId: toolCall.toolCallId,
            metadata: buildToolInputReadyMetadata(toolCall.streamedInput.inputText),
            timestamp: Date.now(),
          })
        }
    ...
    const captureInterruptedPartial = (): void => {
      ...
      const hasPendingToolCalls =
        !isEmpty(pendingToolCalls) || pendingToolInputDrafts.size > 0 || !isEmpty(rejectedToolCalls)
    ...
    try {
      ...
      while (true) {
        const next = await this.readNextStreamPart(streamIterator, args, turnState)
        if (next.done) {
          shouldReturnStreamIterator = args.abortSignal.aborted && shouldReturnStreamIterator
          break
        }
        ...
        const part = chunk
        this.streamDiagnosticHelper.recordStreamPartDiagnostics(diagnostics, part)
        ...
        if (part.type === 'raw') {
          const reasoningText = this.streamDiagnosticHelper.extractReasoningDeltaFromRawChunk(
            part.rawValue
          )
          if (reasoningText) {
          ...
          const visibleText = this.streamDiagnosticHelper.extractVisibleTextFromRawChunk(
            part.rawValue
          )
        ...
        const inputResolution = resolveProviderFinalToolInput(part.input ?? {})
        ...
        pendingToolCalls.push({
          ...
          toolName,
          input,
          ...(streamedInput ? { streamedInput: { inputText: streamedInput.inputText } } : {}),
        })
        args.providerTurnReducer?.apply({
          type: 'tool-started',
          toolCallId,
    ...
    if (!isEmpty(rejectedToolCalls)) {
      ...
      captureInterruptedPartial()
      this.log.warn('provider finalized an invalid tool-call input; rejecting before execution', {
        turn: turnState.turn,
        model: args.model,
        rejectedToolCalls,
    ...
    routeInlineReasoningSegments(flushInlineReasoningTagTail(inlineReasoningTagState))
    flushCurrentText()
    flushPendingToolCalls()

    if (!args.abortSignal.aborted && !turnState.hasVisibleOutput && !turnState.hasToolUse) {
      if (diagnostics.totalChunks > 0) {
        if (shouldRecoverEmptyStreamAsContextPressure(diagnostics, args.contextPressure)) {
          this.log.warn('assistant stream empty under high context pressure', {
            turn: turnState.turn,
            model: args.model,
            diagnostics: this.streamDiagnosticHelper.summarizeStreamDiagnostics(diagnostics),
        ...
        const fallbackText = this.streamDiagnosticHelper.buildNonDisplayableResponseMessage(
          diagnostics,
    ...
    this.emitUsageTelemetry(diagnostics, turnState, args)
    if (!turnState.hasToolUse && isRecoverableOutputTruncationDiagnostic(finishDiagnostic)) {
  ...
  private emitUsageTelemetry(
    ...
    turnState.requestFingerprint = readRequestFingerprintId(args.requestFingerprint)
  ...
  private recordAbnormalFinishReasonDiagnostic(
    diagnostics: StreamDiagnostics,
    providerTurnReducer?: ProviderTurnEventReducer
  ): ReturnType<AgentStreamDiagnosticHelper['buildAbnormalFinishReasonDiagnostic']> {
    const diagnostic = this.streamDiagnosticHelper.buildAbnormalFinishReasonDiagnostic(diagnostics)
  ...
  private createIdleReconnectNotifier(
    ...
    const delayMs = args.idleReconnectDelayMs ?? StreamIdleReconnectNoticeDelayMs
  ...
  private isToolConcurrencySafe(toolName: string, input: Record<string, unknown>): boolean {
