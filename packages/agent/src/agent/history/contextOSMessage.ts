...
export const CompactionSummaryInstruction =
  ...
  '必须从落盘产物（本任务写出的文件、工具结果）重新计算,或用 context:recall 取回原始数据核实;禁止凭模糊印象或摘要补数字。'

/**
 * 置顶证据段的标准说明文（写入合成消息时拼在 marker 后）。
 */
export const PinnedEvidenceInstruction =
  '下面是当前执行的关键证据：文件读取、变更、命令结果与验证状态。需要证据全文时用 context:recall({ ref: 行首的 id })取回。不要重新查询同一文件，除非证据被标记为 reread-required。'
