/**
 * I2 模型蒸馏是上下文治理第二版的第三档器械，也是唯一会产生模型调用成本的一档。
 *
 * 本文件只实现可纯计算的选段、判据、验证与产物拼装。异步请求由 `ContextDistillRunner` 调度，
 * 模型解析由宿主注入的 `ContextDistiller` 完成，使真实账本能够结合录制输出进行离线重放。
 *
 * 蒸馏绝不内联阻塞当前回合：治理周期同步执行 I0 和 I1，随后异步规划蒸馏，产物到下一个周期
 * 边界才应用，从而保证每个周期只重建一次缓存。
 *
 * I1 骨架始终是保底方案，蒸馏只能升级结果，不能造成损失。只有机械器械仍未达到目标水位时才
 * 规划蒸馏；锚点验证失败、超时或异常都会回落到同一批成员的骨架。即使验证成功，锚点行与召回
 * 指针行仍由规则生成，模型只负责叙事正文。
 *
 * `off`、`aux`、`main` 与 `adaptive` 档位共用同一条实现路径，只由 `resolveDistillGate` 决定是否
 * 调用；自适应档还要求缺口、价值密度与预计节省同时达到阈值。
 */
