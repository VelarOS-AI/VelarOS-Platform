...
class ChatSearchRanking {
  ...
  public rankContextText(input: {
    ...
    const matchedPaths = queryPathHints.filter((hint) => {
      ...
      return (
        lowerText.includes(hint.toLowerCase()) || lowerText.includes(basenameHint.toLowerCase())
      )
    })
    ...
    const pathScore = isEmpty(queryPathHints) ? 0 : matchedPaths.length / queryPathHints.length
    ...
    const errorScore = isEmpty(queryErrorTerms) || isEmpty(textErrorTerms) ? 0 : 1
    ...
    return {
      ...
      semanticScore,
      matchReasons,
      pathHints: isEmpty(matchedPaths) ? textPathHints.slice(0, 4) : matchedPaths,
      symbolHints: isEmpty(matchedSymbols) ? textSymbolHints.slice(0, 4) : matchedSymbols,
      recencyScore,
      errorFingerprint,
    }
  }
  ...
  public buildSearchSnippet(
    ...
    const start = !isPresent(firstMatch) ? 0 : Math.max(0, firstMatch - Math.floor(maxChars / 3))
    return chatSearchText.truncate(trimmed.slice(start, start + maxChars), maxChars)
  }
   ...
   */
  public buildRetrievalScoreStats(scores: number[]): ChatContextRetrievalScoreStats {
