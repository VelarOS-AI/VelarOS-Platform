  ...
  return {
    ...
    prepare(input: PatchStrategyInput): PreparedPatch[] {
      ...
      if (op.type === "delete_file") {
        ...
        if (!snap?.exists) throw new WorkspaceError("TARGET_NOT_FOUND", `无法删除不存在的文件：${op.path}`);
        return [patch(op.path, snap.revision, snap.content ?? "", "", "core.text-patch", { op: "delete_file" })];
      }
      if (op.type === "rename_file") {
        const snap = input.snapshot;
        if (!snap?.exists) throw new WorkspaceError("TARGET_NOT_FOUND", `无法重命名不存在的文件：${op.from}`);
        return [
      ...
      if (op.type === "replace_text") {
        ...
        if (!isPresent(start) || !isPresent(end)) {
          throw new WorkspaceError("TARGET_NOT_FOUND", "replace_text 需要已解析目标范围或 oldText");
        }
      ...
      if (op.type === "insert_text") {
        if (!isPresent(start) || !isPresent(end)) throw new WorkspaceError("TARGET_NOT_FOUND", "insert_text 需要已解析目标范围");
      ...
      if (op.type === "insert_text_at_anchor") {
        if (!op.anchorText) throw new WorkspaceError("INVALID_INPUT", "insert_text_at_anchor 需要 anchorText");
      ...
      if (op.type === "delete_text") {
        ...
        if (!isPresent(start) || !isPresent(end)) throw new WorkspaceError("TARGET_NOT_FOUND", "delete_text 需要已解析目标范围或 oldText");
      ...
      throw new WorkspaceError("NOT_SUPPORTED", `不支持的文本操作：${op.type}`);
    },
  };
}
