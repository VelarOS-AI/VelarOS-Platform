...
const LOCAL_KERNEL_CAPABILITIES: SystemCapabilityId[] = [
  'workspace-roots',
  'run-command',
  'open-path',
  'reveal-path',
...
export function createLocalSystemKernel(options: LocalSystemKernelOptions): SystemToolSystemApi {
...
export class LocalSystemKernel implements SystemToolSystemApi {
  ...
  public getOverview() {
    return {
      ...
      providers: [
        {
          id: 'local-node-kernel',
          label: 'Local Node system kernel',
          description: 'Package-owned Node.js implementation for primitive system tools.',
          capabilities: LOCAL_KERNEL_CAPABILITIES,
        },
      ],
      workspaceRoots: this.getWorkspaceRootSnapshot(),
    }
  }
  ...
  public async inspectEnvironment(commands: string[] = []): Promise<SystemEnvironmentInspection> {
    ...
    return {
      ...
      workspace: {
        systemWorkspaceRoot: this.activeWorkspaceRoot,
        activeRoot: this.activeWorkspaceRoot,
        configuredRoots: this.getWorkspaceRootSnapshot(),
        activeConfiguredRoot: this.activeWorkspaceRoot,
      },
  ...
  public async globalSearch(
    ...
    const rootPath = this.resolvePath(options.rootPath ?? this.activeWorkspaceRoot ?? this.homeDir)
  ...
  private getWorkspaceRootSnapshot(): SystemRootEntry[] {
    return [{
      path: this.activeWorkspaceRoot,
      active: true,
      exists: true,
      removable: false,
      source: 'project',
  ...
  public async runCommand(
    ...
    const cwd = this.resolvePath(options.cwd ?? this.activeWorkspaceRoot ?? this.homeDir)
  ...
  public async discoverProjects(
    _options?: SystemProjectDiscoveryOptions
  ): Promise<SystemDiscoveredProject[]> {
    return this.unsupported('discoverProjects')
  }

  public async listRecentProjects(
    _options?: SystemProjectDiscoveryOptions
  ): Promise<SystemRecentProjectEntry[]> {
    return []
  }

  public async inferActiveProject(
    _options?: SystemProjectDiscoveryOptions
  ): Promise<SystemActiveProjectInference> {
    return this.unsupported('inferActiveProject')
  }

  public async getProjectContext(
    _options?: SystemProjectContextOptions
  ): Promise<Nullable<SystemProjectContextSummary>> {
    return null
  }

  public async readRecentLogs(_options?: SystemRecentLogQueryOptions): Promise<SystemRecentLogEntry[]> {
    return []
  }

  public async associateProcessesWithProjects(
    _options?: SystemProcessProjectAssociationOptions
  ): Promise<SystemProcessProjectAssociation[]> {
    return this.unsupported('associateProcessesWithProjects')
  }

  public async summarizeCurrentDevEnvironment(
    _options?: SystemDevEnvironmentSummaryOptions
  ): Promise<SystemDevEnvironmentSummary> {
    return this.unsupported('summarizeCurrentDevEnvironment')
  }

  public async diagnoseDevRuntime(
    _options?: SystemDevRuntimeDiagnosisOptions
  ): Promise<SystemDevRuntimeDiagnosis> {
    return this.unsupported('diagnoseDevRuntime')
  }

  public async getSystemToolPromptHints(): Promise<SystemToolPromptHint[]> {
    return []
  }

  public createSystemToolInstallSuggestion(_input: {
    command: string
    reason: string
  ...
  public async installSystemTool(_request: SystemToolInstallRequest): Promise<SystemToolInstallResult> {
    return this.unsupported('installSystemTool')
  }
  ...
  private tryKillProcessTree(pid: number | undefined, signal: NodeJS.Signals): void {
    if (!pid) return
  ...
  private tryReapProcessTree(pid: number | undefined, signal: NodeJS.Signals): void {
    if (!pid) return
  ...
  private reapForegroundProcessGroupAfterExit(pid: number | undefined, command: string): void {
    if (!pid) return
  ...
  private normalizeExtensions(extensions: string[] | undefined): Set<string> {
    return new Set(
  ...
  private resolvePath(path: string): string {
    ...
    return isAbsolute(normalizedPath)
      ...
      : resolve(this.activeWorkspaceRoot ?? this.homeDir, normalizedPath)
  }
  ...
  private async unsupported<T>(capability: string): Promise<T> {
    throw new AppError(
      'UNSUPPORTED',
      `The local system kernel does not implement ${capability}. Use the Electron host adapter or an extension toolset.`
    )
  }
}
