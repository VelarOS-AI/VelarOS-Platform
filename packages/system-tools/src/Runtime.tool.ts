...
// listProcesses 工具实现已删除（审查报告 m12 死代码清理）：注册表里不再登记，
// 也没有任何 SystemTools.ts re-export，请通过 SystemDomainService.listProcesses
// 或 run_command 工具实现等价能力。
...
const refreshShellEnvironment = defineSystemTool<Record<string, never>>({
  name: 'refresh_shell_environment',
  role: 'control',
...
const terminateBackgroundTask = defineSystemTool<{
  ...
  schema: z.object({
    taskId: z.string().min(1).max(120).describe(
      parameterDescription({
        description: '后台任务 id。',
        usage: ['取自 list_background_tasks 返回的任务条目，不要自己编。'],
      })
    ),
...
const systemRuntimeTools = {
  refresh_shell_environment: refreshShellEnvironment,
  list_background_tasks: listBackgroundTasks,
  terminate_background_task: terminateBackgroundTask,
}
