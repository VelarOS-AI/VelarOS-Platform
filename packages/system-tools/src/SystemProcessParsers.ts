...
export function parseCpuUsagePercent(stdout: string, platform: NodeJS.Platform): number | null {
  if (platform === 'darwin') {
...
export function parseSwapStats(stdout: string, platform: NodeJS.Platform): ByteStats | null {
  if (platform === 'darwin') {
...
export function parseDiskStats(stdout: string, platform: NodeJS.Platform): ByteStats | null {
...
function parseProcessLine(line: string, platform: NodeJS.Platform): ParsedProcessRow | null {
  ...
  if (platform === 'win32') {
    ...
    if (pid === null || ppid === null || !Number.isFinite(cpuPercent) || !Number.isFinite(memoryKb)) return null
...
function parseLsofOpenPortEntries(stdout: string): RawPortEntry[] {
  ...
  let current: RawPortEntry | null = null
...
function parseWindowsOpenPortEntries(stdout: string): RawPortEntry[] {
  ...
  }).filter((entry): entry is RawPortEntry => entry !== null)
}

function parseAddressAndPort(value: string): { address: string; port: number } | null {
...
function parseTabSeparatedByteStats(stdout: string): ByteStats | null {
