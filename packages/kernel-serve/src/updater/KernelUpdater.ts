...
export class KernelUpdater {
  ...
  public async ensureCompatible(
    ...
    const resolved: KernelCompatibilityRequirement = typeof requirement === 'string'
      ? { range: requirement }
      : requirement
  ...
  public async rollback(
    ...
    return this.exclusively(async () => {
      ...
      const target = pointer?.previousVersion ?? undefined
      if (target === undefined) {
        throw new KernelUpdaterError(
          'NO_ROLLBACK_TARGET',
  ...
  private async resolveCompatible(
    ...
    const running = pointer !== undefined
      && acceptable.some((entry) => entry.version === pointer.version)
      ? pointer
      : undefined
    if (running !== undefined) return runningKernelOutcome(running)

    const local = acceptable.at(-1)
    if (local !== undefined) return this.switchTo(local.version, false, requirement)

    if (requirement.allowInstall === false) {
      throw new KernelUpdaterError(
        'VERSION_UNAVAILABLE',
  ...
  private async installVersion(
    ...
    const reusable = existing !== undefined && options.force !== true
  ...
  private async activate(
    ...
    if (await readKernelInstallRecord(path) === undefined) {
      throw new KernelUpdaterError(
    ...
    if (options.healthCheck === undefined) return pointer
  ...
  private async revertPointer(
    previous: KernelActivePointer | undefined,
  ): Promise<void> {
