...
export function createJsTsPatchStrategy(identity: JsTsPatchStrategyIdentity): PatchStrategy {
  return {
    ...
    prepare(input: PatchStrategyInput): PreparedPatch[] {
      ...
      if (!snapshot || snapshot.isBinary || !isPresent(snapshot.content))
        throw new ProjectError('NOT_SUPPORTED', 'JS/TS patch 需要文本 snapshot')
      ...
      if (operation.type === 'add_import' || operation.type === 'remove_import') {
        if (!isJsTsPath(snapshot.path))
          throw new ProjectError(
            'NOT_SUPPORTED',
            `${operation.type} 只支持 JS/TS 文件：${snapshot.path}`,
          )
        const splice =
          operation.type === 'add_import'
        ...
        if (!splice)
          return [
            makePatch(identity.id, snapshot, content, content, { op: operation.type, noop: true }),
          ]
        return [
          makePatch(identity.id, snapshot, content, applySplice(content, splice), {
            op: operation.type,
      ...
      if (!isSymbolOperation(operation))
        throw new ProjectError('NOT_SUPPORTED', `不支持的 JS/TS 操作：${operation.type}`)
...
function symbolSplice(located: LocatedSymbol, operation: SymbolOperation): SourceSplice {
  switch (operation.type) {
    case 'replace_symbol': {
      if (operation.mode !== 'body')
        return {
          start: located.range.startOffset,
          end: located.range.endOffset,
      ...
      return {
        start: located.body.startOffset,
        end: located.body.endOffset,
        text: operation.replacement,
      }
    }
    case 'insert_around_symbol': {
      const at =
        operation.position === 'before' ? located.anchor.startOffset : located.anchor.endOffset
      return { start: at, end: at, text: operation.text }
    }
    case 'insert_before_symbol':
      return {
        start: located.anchor.startOffset,
        end: located.anchor.startOffset,
        text: operation.text,
      }
    case 'insert_after_symbol':
      return {
        start: located.anchor.endOffset,
        end: located.anchor.endOffset,
        text: operation.text,
      }
  }
}
...
function locateSymbol(
  ...
  if (isPresent(range?.startOffset) && isPresent(range.endOffset)) {
    const aligned = isJsTsPath(snapshot.path)
      ? parseJsTs(snapshot.path, content).symbols.find(
          (symbol) =>
            symbol.range.startOffset === range.startOffset &&
            symbol.range.endOffset === range.endOffset,
        )
      : undefined
  ...
  if (!query)
    throw new ProjectError(
      'TARGET_NOT_FOUND',
      `${operation.type} 需要 targetId 或 operation.symbol`,
    )
  if (!isJsTsPath(snapshot.path))
    throw new ProjectError(
      'NOT_SUPPORTED',
      `${operation.type} 按 symbol 定位只支持 JS/TS 文件：${snapshot.path}`,
    )
  ...
  if (isEmpty(matches))
    throw new ProjectError('TARGET_NOT_FOUND', `${operation.type} 未找到符号：${label}`)
  if (matches.length > 1) {
    throw new ProjectError(
      'AMBIGUOUS_TARGET',
...
function locatedFrom(symbol: JsTsSymbol): LocatedSymbol {
  return {
    range: symbol.range,
    anchor: symbol.statementRange ?? symbol.range,
    body: symbol.bodyRange,
  }
}
