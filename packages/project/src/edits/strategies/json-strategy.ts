...
type JsonPatch =
  { op: 'add' | 'replace'; path: string; value: JsonValue } | { op: 'remove'; path: string }
...
function pointerParts(pointer: unknown, patchIndex: number): string[] {
  ...
  if (!pointer.startsWith('/')) {
    patchError(
      ...
      pointer,
      'invalid_pointer',
      `JSON Pointer 必须为空字符串或以 / 开头：${pointer}`,
    )
  }
  return pointer
    .slice(1)
...
function isStrictJsonValue(value: unknown): value is JsonValue {
  ...
  const visit = (candidate: unknown): boolean => {
    ...
    if (
      ...
      !isNull(Object.getPrototypeOf(candidate))
    )
      return false
