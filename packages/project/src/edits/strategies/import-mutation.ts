...
const NextLineDirectivePattern =
  /^\/(?:\/|\*)+\s*(?:@ts-ignore|@ts-expect-error|eslint-disable-next-line|prettier-ignore)\b/

/** 只在文件开头注释里生效的 pragma：三斜线指令、@ts-nocheck / @ts-check、@jsx 系列、@flow、整文件 eslint-disable。 */
const FilePragmaPattern =
  /^\/\/\/\s*<|@(?:ts-nocheck|ts-check|jsx\w*|flow)\b|eslint-disable(?![-\w])/
...
export function planAddImport(
  ...
  if (isPresent(operation.importStatement)) {
    ...
    if (dedupe && imports.some((declaration) => importIdentity(declaration) === identity))
      return null
  ...
  if (isTrue(operation.sideEffectOnly)) {
    ...
    if (dedupe && sameModule.some((declaration) => !declaration.importClause?.isTypeOnly))
      return null
  ...
  if (isPresent(operation.namespaceImport)) {
    ...
    const holder = dedupe
      ? preferValueClause(clauses.filter((clause) => namespaceNameOf(clause) === alias))
      : undefined
    if (!holder)
      return insertStatement(
        sourceFile,
        imports,
        renderImport(module, { namespaceImport: alias }, style),
      )
  ...
  if (!isPresent(operation.defaultImport) && isEmpty(named)) {
    throw new ProjectError(
      'INVALID_INPUT',
      'add_import 的 module 形式需要 sideEffectOnly、named、defaultImport 或 namespaceImport 之一',
    )
  }
  if (!dedupe)
    return insertStatement(
      sourceFile,
      imports,
      renderImport(module, { defaultImport: operation.defaultImport, named }, style),
    )
  ...
  const defaultHolder = preferValueClause(
    clauses.filter(
      (clause) => isPresent(clause.name) && clause.name.text === operation.defaultImport,
    ),
  )
  ...
  for (const spec of named) {
    ...
    const holders = clauses.flatMap((clause) =>
      namedElementsOf(clause)
        .filter((element) => elementKey(element) === key)
        .map((element) => ({ clause, element })),
    )
    ...
    if (
      spec.typeOnly ||
      holders.some((candidate) => !isTypeOnlyElement(candidate.clause, candidate.element))
    )
      continue
  ...
  for (const [clause, elements] of promoted)
    splices.push(...promoteClause(sourceFile, clause, elements))
      ...
      ...(host
        ...
        : [
            insertStatement(
              sourceFile,
              imports,
              renderImport(module, { defaultImport, named: missing }, style),
            ),
          ]),
    )
  }
 ...
 */
export function planRemoveImport(
  path: string,
  content: string,
  ...
  if (isPresent(operation.importStatement)) {
    ...
    return removeStatement(
      sourceFile,
      single(matches, `未找到 import 语句：${requested.text}`, describe),
    )
  }
  ...
  if (!isPresent(operation.name))
    return removeStatement(sourceFile, single(sameModule, moduleMissing, describe))
...
function requireModule(module: Optional<string>, operation: string): string {
  if (!isPresent(module))
    throw new ProjectError(
      'INVALID_INPUT',
      `${operation} 需要 importStatement、moduleSpecifier 或 module`,
    )
  return module
}
...
function parseImportStatement(
  path: string,
  statement: string,
): { text: string; declaration: ts.ImportDeclaration } {
  ...
  if (
    ...
    throw new ProjectError(
      'INVALID_INPUT',
      `importStatement 必须是一条完整的 import 声明：${statement}`,
    )
  }
...
function importIdentity(declaration: ts.ImportDeclaration): string {
  ...
  const named =
    bindings && ts.isNamedImports(bindings)
      ? bindings.elements
          .map((element) => `${element.isTypeOnly ? 'type ' : ''}${elementKey(element)}`)
          .sort()
      : []
...
function dropTypeModifier(sourceFile: ts.SourceFile, element: ts.ImportSpecifier): SourceSplice {
  return {
    ...
    end: (element.propertyName ?? element.name).getStart(sourceFile),
    text: '',
  }
}
 ...
 */
function promoteClause(
  sourceFile: ts.SourceFile,
  clause: ts.ImportClause,
  requested: readonly ts.ImportSpecifier[],
): SourceSplice[] {
  const [keyword, firstBinding] = clause.getChildren(sourceFile)
  const splices = [
    { start: keyword.getStart(sourceFile), end: firstBinding.getStart(sourceFile), text: '' },
  ]
        ...
        : [
            {
              ...
              text: `, { ${missing.map((spec) => spec.text).join(', ')} }`,
            },
          ]),
    )
  }
...
function insertIntoNamedImports(
  ...
  if (!last)
    return [{ start: named.getStart(sourceFile), end: named.end, text: `{ ${texts.join(', ')} }` }]
  const content = sourceFile.text
  const closeBrace = named.end - 1
  const closeLineStart = lineStartOf(content, closeBrace)
  if (!/^[\t ]*$/.test(content.slice(closeLineStart, closeBrace)))
    return [{ start: last.end, end: last.end, text: `, ${texts.join(', ')}` }]
  ...
  const lines = texts.map(
    (text, index) =>
      `${indent}${text}${trailingComma || index < texts.length - 1 ? ',' : ''}${eol}`,
  )
...
function removeBinding(
  ...
  if (binding.kind === 'named') {
    ...
    if (index < elements.length - 1)
      return {
        start: binding.element.getStart(sourceFile),
  ...
  if (binding.kind === 'default' && namedBindings)
    return { start: clause.getStart(sourceFile), end: namedBindings.getStart(sourceFile), text: '' }
  // 删掉的是唯一的 named / namespace 绑定而还有 default 时，只删 `, { … }` / `, * as ns`。
  if (binding.kind !== 'default' && clause.name)
    return { start: clause.name.end, end: clause.end, text: '' }
...
function removeStatement(
  ...
  for (const comment of [
    ...(ts.getLeadingCommentRanges(content, declaration.pos) ?? []),
  ].reverse()) {
...
function headerEnd(sourceFile: ts.SourceFile): Optional<number> {
  ...
  for (const comment of ts.getLeadingCommentRanges(content, 0) ?? []) {
    ...
    if (FilePragmaPattern.test(text) || DetachedCommentPattern.test(content.slice(comment.end)))
      anchor = comment.end
  }
