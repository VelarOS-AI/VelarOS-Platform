...
import {
  adaptTextToContentLineEndings,
  includesLineEndingAware,
  offsetToLine,
  resolveLineEndingAwareTextMatch,
} from '../../utils/text.js'
import {
  describeMatchLines,
...
} from '../text-match-feedback.js'

import { replaceSnapshotLines } from './line-edit.js'
import { createTextPatch as patch } from './prepared-patch.js'
...
function selectMissingTextMatch(
  ...
  const tolerantLocations = locateTextMatches(
    content,
    tolerant.map((match) => match.index),
  )
...
function selectTextMatches(
  ...
  if (isPresent(operation.occurrence)) {
    if (
      ...
      operation.occurrence > indices.length
    ) {
      const { matches, lines } = ambiguity()
      throw new ProjectError(
        'TARGET_NOT_FOUND',
...
function createFilePatch(op: CreateFileOperation, snapshot: Optional<FileSnapshot>): PreparedPatch {
  ...
  return patch(
    op.path,
    snapshot?.revision,
    isPresent(replaced) ? revertibleContent(replaced) : '',
    op.content,
    'core.text-patch',
    {
      op: 'create_file',
  ...
  return {
    ...
    prepare(input: PatchStrategyInput): PreparedPatch[] {
      ...
      if (op.type === 'delete_file') {
        ...
        if (!snap?.exists)
          throw new ProjectError('TARGET_NOT_FOUND', `无法删除不存在的文件：${op.path}`)
        assertFileTarget(snap, 'delete_file')
        return [
          patch(op.path, snap.revision, rebuildableContent(snap), '', 'core.text-patch', {
            op: 'delete_file',
            textEncoding: snap.textEncoding,
          }),
        ]
      }
      if (op.type === 'rename_file') {
        const snap = input.snapshot
        if (!snap?.exists)
          throw new ProjectError('TARGET_NOT_FOUND', `无法重命名不存在的文件：${op.from}`)
        ...
        return [
          patch(op.from, snap.revision, content, '', 'core.text-patch', {
            op: 'rename_file_delete',
            to: op.to,
            textEncoding: snap.textEncoding,
          }),
          patch(op.to, undefined, '', content, 'core.text-patch', {
            op: 'rename_file_create',
            from: op.from,
            textEncoding: snap.textEncoding,
          }),
        ]
      ...
      if (!snap) {
        const pathHint =
          input.target?.path ?? ('path' in op ? op.path : undefined) ?? '(unknown path)'
        throw new ProjectError(
          'NOT_SUPPORTED',
      ...
      if (snap.exists && !isString(snap.content)) {
        throw new ProjectError(
          ...
          `text patch 不能编辑超出读取上限的文件（${snap.path}，${snap.size} 字节）。`,
          {
            path: snap.path,
            size: snap.size,
            maxFileSizeToReadBytes: input.policy.maxFileSizeToReadBytes,
          },
      ...
      if (op.type === 'replace_text') {
        ...
        replacementText = adaptTextToContentLineEndings(
          lineEndingReference(content, replacedText),
          replacementText,
        )
      ...
      if (op.type === 'insert_text') {
        if (!isPresent(start) || !isPresent(end))
          throw new ProjectError('TARGET_NOT_FOUND', 'insert_text 需要已解析目标范围')
      ...
      if (op.type === 'insert_text_at_anchor') {
        if (!op.anchorText)
          throw new ProjectError('INVALID_INPUT', 'insert_text_at_anchor 需要 anchorText')
        const selected = selectTextMatches(
          input,
          op,
          content,
        ...
        if (
          op.skipIfAlreadyPresent &&
          (includesLineEndingAware(content, op.text) || content.includes(insertionText))
        )
          return [
            patch(snap.path, snap.revision, content, content, 'core.text-patch', {
      ...
      if (op.type === 'append_text') {
        ...
        const newContent =
          op.skipIfAlreadyPresent &&
          (includesLineEndingAware(content, op.text) || content.includes(appendedText))
            ? content
      ...
      if (op.type === 'prepend_text') {
        ...
        const newContent =
          op.skipIfAlreadyPresent &&
          (includesLineEndingAware(content, op.text) || content.includes(prependedText))
            ? content
            : prependedText + content
        return [
          patch(snap.path, snap.revision, content, newContent, 'core.text-patch', {
            op: 'prepend_text',
          }),
      ...
      if (op.type === 'delete_text') {
        ...
        if (!isPresent(start) || !isPresent(end))
          throw new ProjectError('TARGET_NOT_FOUND', 'delete_text 需要已解析目标范围或 oldText')
