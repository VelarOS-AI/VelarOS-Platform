...
function alignedEqual(
  ...
  if (index === 0)
    return count === 1 && !endAnchored ? actual.includes(expected) : actual.endsWith(expected)
...
function equalUnder(
  ...
  const bodySteps = steps.filter(
    ([cause]) => cause !== 'line_ending' && cause !== 'trailing_whitespace',
  )
  return alignedEqual(
    normalizeWith(bodySteps, expected),
    normalizeWith(bodySteps, actual),
    index,
    count,
  )
}
...
function alignedAnchorStarts(file: readonly string[], needle: readonly string[]): CandidateStart[] {
  ...
  const hitsOf = (anchor: string, index: number) =>
    index > 0 && index < count - 1
      ? (lineIndex.get(anchor) ?? [])
      : file.flatMap((line, fileIndex) =>
          alignedEqual(anchor, line, index, count) ? [fileIndex] : [],
        )
  const starts = needle
    .map((anchor, index) => ({
      index,
      hits: isBlank(anchor) ? [] : hitsOf(anchor, index).filter((hit) => hit >= index),
    }))
...
function fragmentSimilarity(
  anchorGrams: ReadonlyMap<string, number>,
  anchorLength: number,
  line: string,
): number {
...
function fuzzyAnchorStarts(file: readonly string[], needle: readonly string[]): CandidateStart[] {
  ...
  for (const anchor of anchors) {
    ...
    for (const [fileIndex, line] of file.entries()) {
      ...
      if (similarity >= MinFragmentSimilarity && similarity > (best.get(start) ?? 0))
        best.set(start, similarity)
    }
  }
  return [...best]
...
function bestCandidateStart(
  ...
  const alignedLines = (needle: readonly string[], file: readonly string[], start: number) =>
    needle.filter((line, index) => alignedEqual(line, file[start + index], index, needle.length))
      .length
  ...
  for (const { start, similarity } of [...candidates].sort(
    (left, right) => left.start - right.start,
  )) {
    const rank = [
      alignedLines(semanticNeedle, semanticFile, start),
      alignedLines(looseNeedle, looseFile, start),
      similarity,
    ]
...
function describeIndent(line: string): string {
  ...
  return [tabs > 0 ? `${tabs} 个制表符` : '', spaces > 0 ? `${spaces} 个空格` : '']
...
export function diagnoseTextMatchMiss(content: string, needle: string): TextMatchMissDiagnosis {
  ...
  const toleratedBefore = (lineCount: number) =>
    unique(
      needleLines
        .slice(0, lineCount)
        .flatMap((line, index) =>
          alignedEqual(line, fileLines[start + index], index, count)
            ? []
            : mismatchCauses(line, fileLines[start + index], index, count),
        ),
    )
  // 宽容口径下仍不相等的第一行才是模型需要修正的分歧。
  const mismatchIndex = needleLines.findIndex(
    (line, index) => !equalUnder(TolerantSteps, line, fileLines[start + index], index, count),
  )
  if (mismatchIndex === -1) {
...
export function summarizeTextMatchMiss(diagnosis: TextMatchMissDiagnosis): string {
  ...
  const tolerated = isPresent(diagnosis.tolerated)
    ? '（其间的行尾空白/换行符差异会被自动宽容，无需修正）'
    : ''
  return (
    `最接近位置从文件第 ${diagnosis.candidateLine} 行开始，oldText 前 ${diagnosis.matchedLines} 行一致${tolerated}，` +
...
export function textMatchMissNextAction(diagnosis: TextMatchMissDiagnosis): string {
  ...
  const target = mismatch
    ? `把 oldText 第 ${mismatch.oldTextLine} 行改成文件原文`
    : '按文件原文修正 oldText'
...
export function locateTextMatches(
  content: string,
...
function enclosingLine(lines: readonly string[], lineIndex: number): Optional<string> {
  ...
  for (
    ...
    index >= Math.max(0, lineIndex - MaxEnclosingLookbackLines);
    index -= 1
  ) {
