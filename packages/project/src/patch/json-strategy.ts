...
function pointerParts(pointer: string): string[] {
  ...
  if (!pointer.startsWith("/")) throw new ProjectError("INVALID_INPUT", `JSON pointer 无效：${pointer}`);
...
export function jsonPatchStrategy(): PatchStrategy {
  return {
    ...
    prepare(input: PatchStrategyInput): PreparedPatch[] {
      ...
      if (!snap?.content) throw new ProjectError("TARGET_NOT_FOUND", "JSON patch 需要文本 snapshot");
      let data: any;
      try {
      ...
      for (const p of op.patches) {
        ...
        if (p.op !== "add" && p.op !== "replace" && p.op !== "remove") {
          throw new ProjectError("INVALID_INPUT", `不支持的 json_patch 操作：${String(p.op)}（仅支持 add/replace/remove）`);
        }
