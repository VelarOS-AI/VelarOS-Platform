...
import { isInsideRoot, toAbs, toRel } from '../utils/path.js'
import {
...
import {
  ...
  continuationInput,
  countTextLines,
  detectBinaryByPrefix,
  endColumnClampNote,
  estimateTokensFromBytes,
  hashFileAndDetectBinary,
  limitContent,
  readLimitedTextPrefix,
  readPastEnd,
  readTextLineWindow,
  sliceWindowColumns,
} from './file-reader.js'
...
async function assertConfinedToRoot(
  realRoot: string,
  abs: string,
  inputPath: string,
): Promise<void> {
...
export class FileStore implements ProjectFileAccess {
  ...
  private identity(
    ...
    if (this.policy.revisionStrategy === 'metadata')
      return {
  ...
  /** 按范围、字节数或字符数限制读取文本内容。 */
  public async read(input: ReadInput, options?: { skipFileFilter?: boolean }): Promise<ReadResult> {
    ...
    if (isUndefined(snap.content) && hasRange) {
      ...
      const window = await readTextLineWindow(
        snap.absPath,
        {
    ...
    if (hasRange) {
      ...
      const hasMore =
        limited.truncated || selected.hasMoreOnEndLine || sliced.range.endLine < totalLines
  ...
  public async stat(
    ...
    const endLine = isNull(lineCount)
      ? StatRecommendedMaxLines
      : Math.min(lineCount, StatRecommendedMaxLines)
  ...
  public async listFiles(input: ObserveInput = {}): Promise<FileListEntry[]> {
    return listProjectFiles(input, {
      ...
      authorize: (...args) => this.authorize(...args),
      allowed: (...args) => this.allowed(...args),
    })
  }
  ...
  public async observe(input: ObserveInput = {}): Promise<ProjectSnapshot> {
    ...
    const revision = revisionFor(
      'project',
      sha256(snapshots.map((s) => `${s.path}:${s.sha256}`).join('\n')),
      Date.now(),
      snapshots.length,
    )
