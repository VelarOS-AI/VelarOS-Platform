...
function buildRuntimeToolCapabilityMap(snapshot: RuntimePromptSnapshot): string {
  ...
  if (hasRuntimeTool(snapshot, 'tool_map')) {
    lines.push('缺少能力时先用 tool_map 查看可装载能力，再按返回的 activation 信息处理。')
  }
  if (hasAnyRuntimeTool(snapshot, ['recall_context'])) {
    lines.push('需要恢复已裁剪证据时调用 recall_context。')
  }
  if (hasAnyRuntimeTool(snapshot, ['distill_context'])) {
    lines.push('长任务阶段转折点可调用 distill_context 保存关键事实和进度。')
  }
  return lines.join('\n')
}
...
function createRuntimePromptSegments(
  ...
  segments.push(
    ...
    createTextPromptSegment({
      ...
      when: () => hasRuntimeTool(snapshot, 'dispatch_agent'),
      text: [
