 ...
 *  - capability：能力技能。descriptor 常驻、正文按需 tool_read；命中 feature/选中后只注入 skill:<id> 指针。
 */
type AgentSkillKind = 'role' | 'capability'

/**
 * 智能体技能定义。
 * - 继承技能描述，增加完整文档内容，用于 tool_read 按需读取。
...
interface AgentSkillDefinition extends AgentSkillDescriptor {
  ...
  baseDir?: string
  /** 目录式技能的捆绑资源相对路径清单（渐进披露第三层，tool_read skill:<id>/<路径> 按需读取）。 */
  resourcePaths?: readonly string[]
  /** 项目级技能的来源项目根：仅当会话活跃根等于它时可见（根级精筛）。 */
  sourceResourceId?: string
  /** 用户可暂停技能；禁用后不参与列表、自动命中或 tool_read。 */
  enabled: boolean
   ...
   * 技能可见/可触发的能力作用域。
   *
   * 空数组表示通用；指定范围时，列表、tool_read 和自动注入都只在对应作用域生效。
   */
