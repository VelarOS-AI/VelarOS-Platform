/**
 * 聊天流式起搏器：单个会话一条动画帧链 + 块级串行门控。
 *
 * 后端把「思考 / 工具 / 正文」按 token 交错下发（sequence 交错）。起搏器策略：
 *
 *  - **思考 / 正文 / 工具 / 结构事件全部归并成同一条块 FIFO**：每帧只渲染队首块，
 *    思考与正文都按字符预算逐帧吐字、工具/结构事件逐帧应用一个；队首吐空且后面有更新块时出队。
 *    思考也进 FIFO（去重后入块、不再每个 token 立即全量 apply）——消除"思考流与被起搏的正文
