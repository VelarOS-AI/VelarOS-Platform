  ...
  return (
    ...
    >
      <SelectPrimitive.Trigger
        ...
        className={cn(selectVariants({ variant, size: resolvedSize }), className)}
        aria-label={selected?.label ?? placeholder}
      >
        <span className={'velar-select-trigger-content'}>
          {!!selected?.icon && <span className={'velar-select-trigger-icon'}>{selected.icon}</span>}
            ...
            {options.map((option) => (
              ...
              >
                <span className={'velar-select-item-content'}>
                  {!!option.icon && <span className={'velar-select-item-icon'}>{option.icon}</span>}
                  <span className={'velar-select-item-meta'}>
                    <SelectPrimitive.ItemText asChild>
                      <span className={'velar-select-item-label'}>{option.label}</span>
                    </SelectPrimitive.ItemText>
                    {!!option.description && (
                      <span className={'velar-select-item-description'}>{option.description}</span>
                    )}
                  </span>
                </span>
                <SelectPrimitive.ItemIndicator className={'velar-select-item-indicator'}>
...
export const Select = memo(SelectImpl) as <T extends string>(props: SelectProps<T>) => ReactElement
