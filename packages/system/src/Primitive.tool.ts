...
import { SystemToolNames } from './system-tool-names.js'
import {
...
const read = defineSystemTool<{
  ...
  notes: ['支持绝对路径和 ~；项目根内文件正文使用 project:read。'],
  schema: z
    .object({
...
const grep = defineSystemTool<{
  ...
  notes: [
    '用于系统路径；项目根内正文搜索使用 project:search。',
...
const systemPrimitiveTools = Object.freeze({
  [SystemToolNames.read]: read,
  [SystemToolNames.write]: write,
  [SystemToolNames.edit]: edit,
  [SystemToolNames.list]: list,
  [SystemToolNames.search]: grep,
  [SystemToolNames.run]: bash,
  [SystemToolNames.processes]: ps,
  [SystemToolNames.open]: open,
})
