...
import { CapabilitySessionLedger } from '../runtime/capability-grants'
import { KernelIdentityRegistryError } from '../runtime/identity-registry'
import {
  type KernelService,
  KernelServiceLifecycleError,
} from '../runtime/KernelService'
...
export class KernelLocalRpcServer {
  ...
  public async start(): Promise<KernelRpcEndpoint> {
    ...
    if (this.state === 'listening') return this.getEndpoint()
    ...
    try {
      ...
      await this.options.service.dispose().catch(() => undefined)
  ...
  public getEndpoint(): KernelRpcEndpoint {
    if (this.resolvedEndpoint === undefined) {
  ...
  private dispatch(
    ...
    switch (request.method) {
      ...
      case 'session.get':
        return this.options.service.getSession(
      ...
      case 'run.get':
        return this.options.service.getRun(
...
function readRequestId(input: unknown): string {
  if (typeof input !== 'object' || input === null) return 'invalid-frame'
...
function resolveBoundEndpoint(
  ...
  if (address === null) throw new Error('Kernel RPC server has no address')
  return {
    kind: 'tcp',
    host: '127.0.0.1',
    port: address.port,
