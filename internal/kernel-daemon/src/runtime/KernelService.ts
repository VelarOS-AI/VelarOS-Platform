...
function readCallId(input: unknown): string {
  if (typeof input !== 'object' || input === null) return 'invalid-call'
...
export class KernelService {
  ...
  public constructor(private readonly options: KernelServiceOptions) {
    if (options.kernelVersion.trim().length === 0) {
  ...
  public async start(): Promise<void> {
    if (this.status === 'disposed') {
      throw new Error('Disposed kernel service cannot be started')
    }
  ...
  private async evaluateRequireItem(
    ...
    if (this.findCapabilityById(item.capabilityId) === undefined) return sessionFailure(
  ...
  public async handleCapabilityCall(
    ...
    let service: object
    try {
      service = this.options.host.getService(
        createCapabilityToken(
          capability.token.id,
    ...
    const scope = toSdkScope(request.scope)
    let metadata: KernelCapabilityOperationMetadata | undefined
    try {
      metadata = service.getOperationMetadata(request.operation)
    } catch {
      return failure(
        request.callId,
    ...
    try {
      const reason = resolveKernelCapabilityOperationReason(
        metadata,
        {
      ...
      for (const permission of metadata.permissions) {
        const decision = await this.options.host.requestCapabilityPermission(
          token,
          {
            permission,
            reason: reason
              ?? `Invoke "${request.operation}" on capability "${request.capabilityId}".`,
            ...(scope === undefined ? {} : { scope }),
          },
        )
        if (decision.status === 'denied') return failure(
            request.callId,
            'PERMISSION_DENIED',
  ...
  private findCapabilityById(capabilityId: string):
    | { readonly token: { readonly id: string; readonly version: string } }
    | undefined {
    for (const snapshot of this.options.host.listModules()) {
      const token = snapshot.manifest.provides.find(
        (provided) => provided.id === capabilityId,
      )
  ...
  private requireModCatalog(): KernelModCatalog {
    if (this.options.modCatalog === undefined) {
...
function toSdkScope(
  ...
  return {
    ...
    ...(scope.kind === null ? {} : { kind: scope.kind }),
  }
}
