...
export default defineComponentLibraryEntry({
  ...
  entry: {
    id: 'fields',
    name: 'Input / SearchField / Select / TextSelect / Textarea / NumberInput / Picker / Calendar / Date Range / TimePicker',
    layer: 'UI',
    status: 'ready',
    domain: 'Main window',
    source: '@velaros-ai/ui',
    usage:
      'Search surfaces use SearchField for one transparent input layer and a whole-field focus state. Dense settings forms use Select + Textarea with size="sm". Scheduled tasks compose CalendarDatePicker + TimePicker instead of native datetime popovers. Date ranges that must always have a value set showClearAction={false} instead of remapping Clear to Today. Model and system rows reuse the same primitives—see Recommended usage for source index.',
    avoid:
